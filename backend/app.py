...
from blueprints.private_tutoring.routes.common import DecimalEncoder
import pytz
...
from blueprints.private_tutoring import private_tutoring_bp
...
app.register_blueprint(private_tutoring_bp)
...
CORS(
    ...
    resources={
        r"/*": {
            "origins": [
                "https://dev.sehanib.kr",
                "https://sehanib.kr",
                "https://www.sehanib.kr",
            ],
            "supports_credentials": True,
...
def generateRandomNumber():
    ...
    app.logger.info(f"new attendance_code {attendance_code}")
...
@app.route("/api/get-homework/<subject>", methods=["GET"])
@cross_origin()
@jwt_required()
def get_homework_subject(subject):
    ...
    if not grade:
        # print("Grade not set for student")
        return

    # print("getting homework for subject name", subject)
    ...
    try:
        with connection.cursor() as cursor:
            sql = """
                ...
                WHERE h.subject = %s 
                    ...
                    AND FIND_IN_SET(%s, h.grades) > 0
                    AND (h.dueDate >= CURDATE() OR filtered_shs.submission_date IS NOT NULL);
            """
            cursor.execute(sql, (student_id, subject, student_id, grade))
            homework_list = cursor.fetchall()
            # print("pending homework list!!", homework_list)

            sql = """SELECT homework_id, comment, marks, submission_date, text_attachment, raw_score, total_score, file_name FROM student_homework_submission WHERE student_id = %s
            """
            cursor.execute(sql, (student_id))
            ...
            submission_info = {
                submission["homework_id"]: {
                    ...
                    "textAttachment": submission["text_attachment"],
                    "rawScore": submission["raw_score"],
                    "totalScore": submission["total_score"],
                    "studentFileName": submission["file_name"],
                }
            ...
            homework_status = [
                {
                    ...
                    "submittedDate": (
                        submission_info[hw["id"]]["submittedDate"]
                        if hw["id"] in submission_info
                        else None
                    ),
                    "textAttachment": (
                        submission_info[hw["id"]]["textAttachment"]
                        if hw["id"] in submission_info
                        else None
                    ),
                    "studentFileName": (
                        submission_info[hw["id"]]["studentFileName"]
                        if hw["id"] in submission_info
                        else None
                    ),
                    "rawScore": (
                        submission_info[hw["id"]]["rawScore"]
                        if hw["id"] in submission_info
                        else None
                    ),
                    "totalScore": (
                        submission_info[hw["id"]]["totalScore"]
                        if hw["id"] in submission_info
                        else None
                    ),
        ...
        connection.close()
    print("homework status", homework_status)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql_time_slot = """
                ...
                FROM time_slots
                WHERE %s BETWEEN DATE_SUB(start_time, INTERVAL 5 MINUTE) AND DATE_SUB(end_time, INTERVAL 5 MINUTE)
                AND grade_id = %s
            """
...
def get_student_attendance_rate():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            rate_today = cursor.fetchone()["classes_passed"]
...
def get_student_homework_rate():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            applicable_homework = cursor.fetchone()
            total_homework_count = applicable_homework["total_homework"]
            ...
            submitted_count = cursor.fetchone()["submitted_homework"]
...
def get_pending_homework_list():
    ...
    try:
        with connection.cursor() as cursor:
            sql = """
                SELECT h.id, h.title, h.assignedDate, h.dueDate, h.subject, s.fg_color
                FROM homework h
                LEFT JOIN student_homework_submission shs
                ...
                AND (
                    FIND_IN_SET(ss.level, h.levels) > 0
                    OR h.levels = 'SL,HL'
                )
                AND (
                    FIND_IN_SET(%s, h.grades) > 0
                    OR h.grades = 'pre-IB,11,12'
                )
                AND h.dueDate >= CURDATE();
            """
            cursor.execute(sql, (student_id, student_id, grade))
            data = cursor.fetchall()
...
# parent-studnet-match update BUT NOT USING. ALL UPDATES JUST USE INSERT
@app.route("/api/parent-student-match/update", methods=["PUT"])
@jwt_required()
def update_parent_student_matches():
...
# insert parent student matches
@app.route("/api/parent-student-match/insert", methods=["POST", "PUT"])
@jwt_required()
def insert_parent_student_matches():
    connection = get_db_connection()

    data = request.json
    print("\nparent student match insert data", data)
    ...
    try:
        with connection.cursor() as cursor:
            sql = """
                DELETE FROM parents_students WHERE parent_id = %s;
            """
...
@app.route("/api/parent-student-match/delete", methods=["DELETE"])
@jwt_required()
def delete_parent_student_matches():
    data = request.json
...
# get parent student matches
@app.route("/api/parent-student-match/get", methods=["GET"])
@jwt_required()
def get_parent_student_matches():
    connection = get_db_connection()
    ...
    try:
        with connection.cursor() as cursor:
            ...
            print("matches in parents students", matches)
...
def oauth_url_api():
    ...
    return redirect(
        f"https://zoom.us/oauth/authorize?response_type=code&client_id=zmBmqzTWi3hfm77oblxA&redirect_uri=https://dev.sehanib.kr/oauth/zoom"
    )
...
def oauth_api():
    ...
    data = {
        ...
        "redirect_uri": "https://dev.sehanib.kr/oauth/zoom",
    }
...
def check_attendance_missing(start_time, grade, subject_name, level_id, mode_id):
    ...
    try:
        with conn.cursor() as cursor:
            # Calculate the start time for this check
            current_time = get_current_time_kst().time()
            check_time = (
                datetime.combine(datetime.today(), start_time) + timedelta(minutes=5)
            ).time()
...
def mark_students_absent(
    ...
    try:
        with conn.cursor() as cursor:
            ...
            app.logger.info(
                f"Current time: {current_time}, Check time: {check_time}, Start time: {start_time}, absent_date: {absent_date}"
            )
            ...
            for student in absent_students:
                ...
                """,
                    (student["student_id"], subject_name, absent_date, timetable_id),
                )
                app.logger.info(
...
def schedule_attendance_checks():
    ...
    app.logger.info(f"Current time: {0}".format(get_current_time_kst()))
    ...
    try:
        with conn.cursor() as cursor:
            ...
            for slot in time_slots:
                ...
                # Schedule attendance code 5 minutes before start of class
                day_scheduler[day_name]().at(
                    (
                ...
                # Schedule checks 10 minutes after and 30 minutes after the start of class
                day_scheduler[day_name]().at(
                    (
                        datetime.combine(datetime.today(), start_time)
...
@app.route("/webhook/test/zoom", methods=["POST"])
def zoom_webhook():
        ...
        for file in recording_files:
            ...
            if file["file_type"] == "MP4":
                ...
                if result:
                    ...
                    file_name = f"{result['grade']}_{result['subject_name']}_{level}.{file_extension}"
