...
export default function Component() {
  ...
  useEffect(() => {
    const fetchTimetables = async () => {
      try {
        ...
        if (error.response) {
          alert(error.response.data.message || "Failed to fetch timetables");
        } else {
          alert("There was an error fetching timetables");
        }
  ...
  const handleSubmit = async (event) => {
    ...
    // Ensure modeID is a number (it should already be "1" or "2" from the select)
    const modeIDNum = parseInt(modeID, 10);
    if (isNaN(modeIDNum) || (modeIDNum !== 1 && modeIDNum !== 2)) {
      alert("Please select a valid mode (Online or Offline)");
      return;
    }

    // Validate that timeSlots is not empty
    if (!timeSlots || timeSlots.length === 0) {
      alert("Please add at least one time slot");
      return;
    }

    // Validate timetable entries - ensure teacher_id is a number
    const validatedTimetables = timetables.map(dayObj => ({
      ...dayObj,
      timeSlots: dayObj.timeSlots.map(slotObj => ({
        ...slotObj,
        entries: slotObj.entries.map(entry => ({
          ...entry,
          teacher_id: entry.teacher_id ? parseInt(entry.teacher_id, 10) : null,
    ...
    try {
      const response = await instance.post("/api/timetable", {
        grade: activeGrade,
        modeID: modeIDNum,
        timeSlots,
        timetables: validatedTimetables,
      });
      ...
      const errorMsg = error.response?.data?.message || error.message || "Failed to save timetables.";
                                ...
                                {getEntries(day, slot.time_slot_id).map(
                                  (value, idx) => (
                                    ...
                                    >
                                      ...
                                      <Select
                                        value={value.teacher_id ? String(value.teacher_id) : ""}
                                        onValueChange={(newValue) =>
                                          handleClassChange(
                                            day,
                                            slot.time_slot_id,
